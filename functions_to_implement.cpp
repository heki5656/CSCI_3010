...
/*std::vector<std::string> Split(std::string whole, std::string separator){
    ...
    //std::int sepLen = separator.length();
    int curr = 0;
    for(int i = 0; i < whole.length(); i++){
        tempSubStr = whole.substr(i, separator.length()-1);
        if(tempSubStr == whole.substr(i, separator.length()-1){
            tempSubStr = whole.substr(i, separator.length()-1);
            stringVec.pushback(whole.substring(curr, i));
            curr = i+1;
        }
    }
    return stringVec;
...
std::string Join(std::vector<std::string> pieces, std::string glue){
    std::string newStr = "";
    int tempSize = pieces.size();
    for(int i = 0; i < tempSize; i++){
        newStr = newStr + glue;
        newStr = newStr + pieces[i];
    }
    return newStr;
}
